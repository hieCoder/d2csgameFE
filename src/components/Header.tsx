          ...
          <div className="flex justify-between items-center">
            ...
            <div className="md:hidden flex items-center">
              ...
              <button className="outline-none mobile-menu-button" onClick={() => setIsOpen(!isOpen)}>
                {isOpen ? <X className="h-6 w-6 text-gray-500" /> : <MenuIcon className="h-6 w-6 text-gray-500" />}
              </button>
            </div>
          </div>
        </div>
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              <form onSubmit={handleSearch} className="relative mx-auto text-gray-600 mb-4">
                <input
              ...
              {menuItems.map((item) => (
                <Link
                  ...
                  className={`block px-3 py-2 rounded-md text-base font-medium ${
                    pathname === item.href ? 'text-white bg-green-500' : 'text-gray-500 hover:text-white hover:bg-green-500'
                  }`}
